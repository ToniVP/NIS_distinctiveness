...
pval <- ggplot(tot_pval_noR, aes(fill=significance, y=value, x=variable)) + 
  geom_bar(position="fill", stat="identity") + scale_fill_manual(values = c("mistyrose1", "lightcoral", "darkred", "darkgreen", "royalblue4"))+
...
sing_NIS <- ggplot() + 
  ...
  scale_color_manual(values= c("firebrick", "goldenrod3", "forestgreen", "blue2", "orchid3"), 
                     name="Species", labels=levels(as.factor(var$fit)))+
  new_scale_color()+
  geom_line(data = var, aes(x, y, group = fit, colour = fit),  size = 0.7) + 
  theme_minimal() + 
  scale_color_manual(values= c("darkred", "darkgoldenrod", "darkgreen", "darkblue", "darkorchid"), name="Species", labels=levels(as.factor(var$fit))) +
